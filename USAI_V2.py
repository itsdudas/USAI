...
def update_interface(imagens, nomes_imagens, nok_folder, df):
    ...
    contagem_reintroducoes = contar_repeticoes(nomes_imagens, df)
    ...
    for imagem, nome_imagem in zip(imagens[:8], nomes_imagens[:8]):
        ...
        if 'Recobrimento' in defeito:
            label_REC = ttk.Label(subframe, text='O que fazer: Verificar limpeza da lente, direção do fio de solda e bico.', font=("Lato", 8), foreground="#FF0000", background="#FFFFFF")
            label_REC.pack(side='bottom', padx=18, pady=0)

        if 'Defeito US' in defeito:
            label_DUS = ttk.Label(subframe, text="O que fazer: Limpar a sonda com cuidado. Verificar peça padrão e masters verde e amarelo.", font=("Lato", 8), foreground="#FF0000", background="#FFFFFF")
            label_DUS.pack(side='bottom', padx=18, pady=0)

        if 'Colagem' in defeito:
            label_COL = ttk.Label(subframe, text="O que fazer: Verificar limpeza da lente, banho da maq. de lavar e decapagem a laser.", font=("Lato", 8), foreground="#FF0000", background="#FFFFFF")
            label_COL.pack(side='bottom', padx=18, pady=0)

        if 'Profundidade' in defeito:
            label_PRO = ttk.Label(subframe, text="O que fazer: Verificar limpeza da lente de proteção, verificar potência e lente do laser. \n (Realizado pela Manuetenção)", font=("Lato", 8), foreground="#FF0000", background="#FFFFFF")
...
def Stats_window():
    ...
    total_rows = len(df)
    total_reintroducoes = df.iloc[:, 1:6].apply(lambda col: (col > 0.5).sum()).sum()
    percentage = (total_reintroducoes / total_rows) * 100
    ...
    info_frame.pack(fill=tk.X, padx=10, pady=(5, 10))  # Adicionar espaçamento

    # Calcular o total de reintroduções
    total_reintroducoes = df.iloc[:, 1:6].apply(lambda col: (col > 0.5).sum()).sum()

    # Label para exibir o total de reintroduções
    ttk.Label(info_frame,
               text=f"Nº Total de reintroduções: {total_reintroducoes} ({percentage:.2f}%)",
               background='#96FF54',
               font=("Lato", 12)).pack(pady=(0,0))

    frame_imagem = tk.Frame(defectsstats_window)
    frame_imagem.pack(pady=(1, 2))
