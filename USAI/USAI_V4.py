...
def process_images():
    ...
    while True:
        ...
        if os.path.exists(txt_file_path):
            ...
            image_to_result = dict(zip(data['Time'], data['Result']))
        else:
        ...
        for image_file in os.listdir(test_images_folder):
            ...
            if image_file.lower().endswith('.jpeg'):
                if image_file in registered_images:
                    continue
                # Obter o valor da coluna 'Result' associado à imagem
                time_group = image_file.split('_')[6].split('.')[0]  # Extrair o grupo '22h10min42s' do nome do arquivo
                if image_to_result.get(time_group) == 'Good part':
...
def Stats_window():
    ...
    ttk.Label(info_frame,
               text=f"Nº Total de Reintroduções: {total_duplicacoes} ({percentage:.2f}%)",
               background='#96FF54',
